...
--------------------|-----------------------------------
Agricultural land   | as a % of land area
CO2 emision         | metric tons per capita
...
--------------|---------------------
country.name | Names of the 134 countries in the data set
country.code | 3-letters code for every country
indicator.name | Name of the 15 indicators 
indicador.code | code with format XXX.XXX.XXX.XX 
year | From 1992 to 2011
value | of the indicator that year in that country
...
Next each one of the indicators are read into a data frame from the downloaded CSV files
```{r, warning=FALSE,message=FALSE}
...
```
Next all data.frames are put into a list
```{r, warning=FALSE,message=FALSE}
...
wdi.df = data.frame()
```
We then loop the list to set year and value as a column and concatenate them. Then rename the variables to the followng below:
...
Make the `indicator.code`, `country.code` variable into a factor and the `year` variable into an ordered factor:
```{r, warning=FALSE,message=FALSE}
...
```
Next create wdi.dfv2 20 years and 134 countries without any missing values
```{r, warning=FALSE,message=FALSE}
