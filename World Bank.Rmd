...
Next, we generate quantiles for each indicator and store it in a different indicator with the same name and `.q`:
```{r}
...
What countries have had the biggest (top quartile) GOP growth and in which years? :
```{r}
wdi.df %>% filter(., GDP.growth.q == 'Q4') %>% select(., country.code, year, GDP.growth, GDP) %>% as.data.frame
```

China has been in the top quantile most of the years (not in 1998, 1999, 2001). India started showing up recently and Russia was at the top quantile in 2000 and 2007.

What countries and in which years have had urban population in the top quartile? :
```{r}
wdi.df %>% filter(., pop.urban.q == 'Q4') %>% select(., country.code, year, pop.urban, pop.urban.q, pop.rural, pop.rural.q) %>% as.data.frame
```

As we can see Brazil has been on top of urban population for all 20 years, whereas Russia entered the top quartile after 2007. 


##References
[1] Investopedia, http://www.investopedia.com/articles/03/042303.asp<br>
[2] Population Reference Bureau, http://www.prb.org/Publications/Articles/2002/HowManyPeopleHaveEverLivedonEarth.aspx <br>
[3] BRICS (Wikipedia), https://en.wikipedia.org/wiki/BRICS <br>
[4] INTERNATIONAL ENERGY OUTLOOK 2016, https://www.eia.gov/forecasts/ieo/emissions.cfm <br>
[5] What China's Economic Growth Figures Mean, http://www.nytimes.com/2016/07/15/business/international/china-gdp-economic-growth.html?_r=0 <br>
[6] World Population 2015, United Nations, https://esa.un.org/unpd/wpp/Publications/Files/World_Population_2015_Wallchart.pdf <br>
