...
```
We create a `data.frame` called `wdi.df`. Then we loop every indicator-specific data frame and continue to:
...
Next, we generate quantiles for each indicator and store it in a different indicator with the same name and a `.q` at the end it. The quantiles will be used for our second part of data analysis:
```{r}
